...
class FavoriteController extends Controller
{
    ...
    public function add_to_favorite_product($pid)
    {
        ...
        {
            return redirect('/')->with('status', 'این محصول قبلا به لیست علاقه مندی های شما اضافه شده!');
        }
        else
        {
            ...
            if($product)
            {
                ...
                return redirect('/')->with('status', 'محصول مورد نظر با موفقیت به لیست علاقه مندی های شما اضافه شد!');
            }
        }
    }
    ...
    {
        //$f = new FavoriteController();
        $this->delete_product_from_favorite_cart($pid);

        $b = new BasketController();
        $b->add_to_basket($pid);

        return redirect('/')->with('status', 'محصول مورد نظر با موفقیت از لیست علاقه مندی ها به سبد خرید اضافه شد!');
    }
        ...
        if(in_array($prod_id_is_there, $item_id_list))
        {
            foreach($cart_data as $keys => $values)
            {
                if($cart_data[$keys]["item_id"] == $pid)
                {
                    ...
                    $minutes = 60;
                    Cookie::queue(Cookie::make('favorite_product_cart', $item_data, $minutes));
                    return redirect('/')->with('status', 'محصول مورد نظر با موفقیت از لیست علاقه مندی های شما حذف شد!');
                }
            }
        }
    }
    ...
    public function add_to_favorite_weblog($wid)
    {
        ...
        if(in_array($prod_id_is_there, $item_id_list))
        {
            return redirect('/')->with('status', 'این مطلب قبلا به لیست علاقه مندی های شما اضافه شده!');
        }
        else
        {
            ...
            if($weblog)
            {
                ...
                return redirect('/')->with('status', 'مطلب مورد نظر با موفقیت به لیست علاقه مندی های شما اضافه شد!');
            }
        }
    }
        ...
        if(in_array($prod_id_is_there, $item_id_list))
        {
            foreach($cart_data as $keys => $values)
            {
                if($cart_data[$keys]["item_id"] == $wid)
                {
                    ...
                    $minutes = 60;
                    Cookie::queue(Cookie::make('favorite_weblog_cart', $item_data, $minutes));
                    return redirect('/')->with('status', 'مطلب مورد نظر با موفقیت از لیست علاقه مندی های شما حذف شد!');
                }
            }
        }
    }
